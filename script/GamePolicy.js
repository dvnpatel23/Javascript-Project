function GamePolicy() {
    ...
    let player1TotalScore = new Score(new trig(Game.size.x / 2 - 75, Game.size.y / 2 - 45));
    let player2TotalScore = new Score(new trig(Game.size.x / 2 + 75, Game.size.y / 2 - 45));

    let player1MatchScore = new Score(new trig(Game.size.x / 2 - 280, 108));
    let player2MatchScore = new Score(new trig(Game.size.x / 2 + 230, 108));

    this.players = [new PoolPlayer(player1MatchScore, player1TotalScore), new PoolPlayer(player2MatchScore, player2TotalScore)];
    this.foul = false;
    this.scored = false;
    this.won = false;
    ...
    this.topCenterHolePos = new trig(750, 32);
    this.bottomCenterHolePos = new trig(750, 794);
    this.topLeftHolePos = new trig(62, 62);
    this.topRightHolePos = new trig(1435, 62);
    this.bottomLeftHolePos = new trig(62, 762)
    this.bottomRightHolePos = new trig(1435, 762);
}
...
GamePolicy.prototype.drawScores = function () {
    Canvas2D.drawText("PLAYER " + (this.turn + 1), new trig(Game.size.x / 2 + 40, 200), new trig(150, 0), "#096834", "top", "Impact", "70px");
...
GamePolicy.prototype.handleBallInHole = function (ball) {
    ...
    if (currentPlayer.color === ball.color) {
        ...
        if (currentPlayer.color != undefined) {
            ...
            let ballsSet = Game.poolGame.getBallsSetByColor(currentPlayer.color);
...
GamePolicy.prototype.updateTurnOutcome = function () {
    ...
    setTimeout(function () { Game.poolGame.whiteBall.visible = true; }, 200);
...
GamePolicy.prototype.handleFoul = function () {
    ...
    if (!Mouse.left.down) {
        Game.poolGame.whiteBall.position = Mouse.position;
    }
