...
function PoolGame() {
    ...
    this.whiteBallStartingPosition = new trig(413, 413);

    this.redBalls = [
        new BilliardsBall(new trig(1056, 433), Color.red),//3
        new BilliardsBall(new trig(1090, 374), Color.red),//4
        new BilliardsBall(new trig(1126, 393), Color.red),//8
        new BilliardsBall(new trig(1126, 472), Color.red),//10;
        new BilliardsBall(new trig(1162, 335), Color.red),//11
        new BilliardsBall(new trig(1162, 374), Color.red),//12
        new BilliardsBall(new trig(1162, 452), Color.red)//14
    ]

    this.yellowBalls = [
        new BilliardsBall(new trig(1022, 413), Color.yellow),//1
        new BilliardsBall(new trig(1056, 393), Color.yellow),//2
        new BilliardsBall(new trig(1090, 452), Color.yellow),//6
        new BilliardsBall(new trig(1126, 354), Color.yellow),//7
        new BilliardsBall(new trig(1126, 433), Color.yellow),//9
        new BilliardsBall(new trig(1162, 413), Color.yellow),//13
        new BilliardsBall(new trig(1162, 491), Color.yellow)//15
    ];

    this.whiteBall = new BilliardsBall(new trig(413, 413), Color.white);
    this.blackBall = new BilliardsBall(new trig(1090, 413), Color.black);
    ...
    this.stick = new CueStick({ x: 413, y: 413 });

    this.gameOver = false;
}

PoolGame.prototype.getBallsSetByColor = function (color) {
...
PoolGame.prototype.handleInput = function (delta) {
...
PoolGame.prototype.ballInHand = function () {
...
PoolGame.prototype.whiteBallOverlapsBalls = function () {
...
PoolGame.prototype.ballsMoving = function () {
...
PoolGame.prototype.handleCollision = function (ball1, ball2, delta) {
    ...
    if (dist < BALL_SIZE) {
        ...
        var velocity2 = new trig(90 * Math.cos(rotation + Math.PI) * power, 90 * Math.sin(rotation + Math.PI) * power);
        ball2.velocity = ball2.velocity.addTo(velocity2);

        ball2.velocity.multiplyWith(0.97);

        var velocity1 = new trig(90 * Math.cos(rotation) * power, 90 * Math.sin(rotation) * power);
...
PoolGame.prototype.draw = function () {
    Canvas2D.drawImage(images.background);
...
PoolGame.prototype.reset = function () {
    this.gameOver = false;
...
PoolGame.prototype.initiateState = function (balls) {
